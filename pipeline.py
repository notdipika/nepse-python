...
def main():
    parser = argparse.ArgumentParser(
        description="NEPSE ETL Pipeline  —  manual runner",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
MANUAL USAGE (default):
  python pipeline.py                       # report from current raw.csv (NO fetch)
  python pipeline.py --email you@gmail.com # same, but also email the report
  python pipeline.py --demo                # generate synthetic data then report

FETCH (usually handled automatically by scheduler.py):
  python pipeline.py --fetch               # run fetch loop only (blocks until 15:00 NPT)
  python pipeline.py --fetch --report      # fetch all day, then report automatically

NOTE: The scheduler handles --fetch automatically between 11:00–15:00 NPT.
      Use this script to generate reports manually at any time.
""",
    )
    ...
    parser.add_argument("--fetch",   action="store_true",
                        help="Run intraday fetch loop (blocks until 15:00 NPT)")
    parser.add_argument("--report",  action="store_true",
                        help="Generate report after --fetch (only used with --fetch)")
    ...
    if args.demo:
        ...
        # Explicit fetch (normally done by scheduler automatically)
        run_pipeline(syms, fetch=True, report=args.report,
                     email_recipients=recip, email_cc=cc)

    else:
        # DEFAULT: just report from whatever is in raw.csv right now
        generate_report_now(email_recipients=recip, email_cc=cc)
